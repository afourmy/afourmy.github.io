...
Before starting, also read the courses already redone and use them as worked examples of the target result: currently `continuite.html`, `complexes.html`, `equations-differentielles.html` and `probabilites.html` (structure, prose register, figures); more will join it over time.
